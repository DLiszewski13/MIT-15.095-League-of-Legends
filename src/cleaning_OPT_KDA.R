...
faker <- lol_data_new %>% select(c(gameid,side,player,position,kills,deaths,assists)) %>% filter(position == 'mid') %>%
          rename(kills_mid = kills,deaths_mid = deaths,assists_mid = assists)
faker
...
match_data_faker = left_join(match_data,faker,by = c('gameid','side'))
...
############# Export data
match_data_faker$playoffs <- as.factor(match_data_faker$playoffs)
match_data_faker$result <- as.factor(match_data_faker$result)
match_data_faker$patch <- as.factor(match_data_faker$patch)
match_data_faker$year <- as.factor(match_data_faker$year)
match_data_faker$month <- as.factor(match_data_faker$month)
match_data_faker$day <- as.factor(match_data_faker$day)
str(match_data_faker)

match_data_cleaned <- match_data_faker %>% select(-c('game', 'agg_period', 'date', 'datetime','gameid' ))
...
# Export data to CSV
write.csv(train_2y1y,"/Users/pan/Dropbox (MIT)/ML_league/data/train_2y1y_2row_faker_OPT.csv", row.names = FALSE)
write.csv(test_2y1y,"/Users/pan/Dropbox (MIT)/ML_league/data/test_2y1y_2row_faker_OPT.csv", row.names = FALSE)
